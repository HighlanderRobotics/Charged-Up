    ...
    addCommands(
        ...
        swerveSubsystem
            .poseLockDriveCommand(
                ...
                true,
                false)
            .finallyDo((boolean interrupt) -> System.out.println("end scoring"))
            // swerveSubsystem.driveCommand(() -> adjustmentSupplier.getAsDouble(), () -> 0, () ->
            // 0, false, false)
            .alongWith(
                new WaitUntilCommand(
                        () -> (swerveSubsystem.getNearestGoalDistance() < 0.05)) // .alongWith(
                    .andThen(
                        ...
                        new PrintCommand(
                            swerveSubsystem.checkIfConeGoal(swerveSubsystem.getNearestGoal())
                                + " nearest goal is cone"),
                        superstructureSubsystem
                            .waitExtendToGoal(level, 0.0)
                            .andThen(
                                new PrintCommand("extended elevator"),
                                new WaitCommand(0.25),
                                new ConditionalCommand(
                                    greybotsGrabberSubsystem.outakeCubeCommand(),
                                    greybotsGrabberSubsystem.outakeConeCommand(),
                                    () ->
                                        swerveSubsystem.checkIfConeGoal(
                                            swerveSubsystem.getNearestGoal())))
